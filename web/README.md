...
2. The browser uploads directly to S3-compatible storage. File bytes never pass through the web
   process.
3. The route confirms the object with `HeadObject` and marks it `ready` only when its metadata
   matches.
4. Downloads re-check organization ownership and redirect to a short-lived signed attachment URL.
